...
def userinfo(request) :
    if request.method == 'GET' :
        ...
        choice_movie_info = []
        for movie in movies:
            genres2 = movie.genres.all()  
            genre_names = [genre.name for genre in genres2]
            if choiceGenre in genre_names:
                choice_movie_info.append(movie.title) 
                # print(f"{movie.title}의 장르: {', '.join(genre_names)}")
        
        pick_movie = random.choice(choice_movie_info)
...
def movie_review_detail(request, movie_review_pk):
    ...
    if request.method == 'GET':
        ...
        if not request.user.movie_review.filter(pk=movie_review_pk).exists():
...
def interests(request, movie_pk):
    ...
    movie = get_object_or_404(Movie, pk=movie_pk)
    if request.method == 'GET':
        if request.user in movie.uninterest_users.all():
        ...
        if request.user in movie.uninterest_users.all():
            movie.uninterest_users.remove(request.user)
            is_uninterested = False
            
        else:
            movie.uninterest_users.add(request.user)
            is_uninterested = True
            # Json field 관리
            for genre in genres:
                genre = get_object_or_404(Genre,name=genre)
                person_genre_dict[str(genre.id)] = person_genre_dict[str(genre.id)] // 2

    context = {
        'is_uninterested': is_uninterested,
    }
