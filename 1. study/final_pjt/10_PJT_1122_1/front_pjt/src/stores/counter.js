...
export const useCounterStore = defineStore('counter', () => {
  ...
  const getGenre = function() {
    axios({
      ...
      .then((res)=>{
        ...
        console.log(genres.value[0].name)
      })
      .catch((err)=>{
        console.log(err)
      })
  ...
  const getUserInfo = function() {
    axios({
      ...
      .then((res)=>{
        ...
        console.log(userInfo.value.user_info.id)
      })
      .catch((err)=>{
        console.log(err)
      })
  ...
  return { movies, API_URL, getMovies, signUp, logIn, token, isLogin, getReviews, reviews, genres, getGenre, getUserInfo, userInfo}
}, { persist: true })
